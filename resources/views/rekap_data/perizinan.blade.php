    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Rekap Data Perizinan</h3>
            </div>
            @if(session('success'))
            ...
            <div class="card-body border-bottom py-3">
                <form action="{{ route('report.perizinan') }}" method="GET" class="d-flex w-100" id="filterForm">
                    ...
                    <div class="ms-auto text-muted d-flex">
                        ...
                        <a href="{{ route('report.perizinan.export', request()->all()) }}" class="btn btn-success btn-sm ms-2">
                            Export Excel
                        </a>
                        <a href="{{ route('report.perizinan.pdf', request()->all()) }}" class="btn btn-danger btn-sm ms-2">
                            Export PDF
                        </a>
                    </div>
                </form>
                        ...
                        <div class="modal modal-blur fade" id="modal-edit-{{ $val->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    <form action="{{ route('report.perizinan.update', $val->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="modal-header">
                                            <h5 class="modal-title">Edit Data Perizinan</h5>
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="jenis_kendaraan" class="form-select" required>
                                                    <option value="Motor Pribadi" {{ $val->jenis_kendaraan == 'Motor Pribadi' ? 'selected' : '' }}>Motor Pribadi</option>
                                                    <option value="Mobil Pribadi" {{ $val->jenis_kendaraan == 'Mobil Pribadi' ? 'selected' : '' }}>Mobil Pribadi</option>
                                                    <option value="Motor Dinas" {{ $val->jenis_kendaraan == 'Motor Dinas' ? 'selected' : '' }}>Motor Dinas</option>
                                                    <option value="Mobil Dinas" {{ $val->jenis_kendaraan == 'Mobil Dinas' ? 'selected' : '' }}>Mobil Dinas</option>
                                                    <option value="Jalan Kaki" {{ $val->jenis_kendaraan == 'Jalan Kaki' ? 'selected' : '' }}>Jalan Kaki</option>
                                                </select>
                                            </div>
                                            <div class="row">
                                                <div class="col-lg-6">
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="datetime-local" name="keluar" class="form-control" value="{{ $val->keluar ? Carbon\Carbon::make($val->keluar)->format('Y-m-d\TH:i') : '' }}" required>
                                                    </div>
                                                </div>
                                                <div class="col-lg-6">
                                                    <div class="mb-3">
                                                        <label class="form-label">Waktu Masuk</label>
                                                        <input type="datetime-local" name="masuk" class="form-control" value="{{ $val->masuk ? Carbon\Carbon::make($val->masuk)->format('Y-m-d\TH:i') : '' }}" required>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
            ...
            <div class="card-footer d-flex align-items-center">
                ...
                <ul class="pagination m-0 ms-auto">
                    <li class="page-item">
                        <a class="page-link" href="{{$controller->prevPagination($report->currentPage(), 'report.perizinan', request()->all())->link}}">
                    ...
                    @foreach($controller->counterPagination($report->lastPage(), $report->currentPage(), 'report.perizinan', request()->all()) as $val)
                    <li class="page-item {{$val->is_active ? 'active' : ''}}"><a class="page-link" href="{{$val->link}}">{{$val->lable}}</a></li>
                    @endforeach
                    <li class="page-item">
                        <a class="page-link" href="{{$controller->nextPagination($report->currentPage(), $report->lastPage(), 'report.perizinan', request()->all())->link}}">
                            next
