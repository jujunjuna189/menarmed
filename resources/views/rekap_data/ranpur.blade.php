    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body border-bottom py-3">
                <form action="{{ route('report.ranpur') }}" method="GET" class="d-flex w-100" id="filterForm">
                    ...
                    <div class="ms-auto text-muted d-flex">
                        ...
                        <a href="{{ route('report.ranpur.export', request()->all()) }}" class="btn btn-success btn-sm ms-2">
                            Export Excel
                        </a>
                        <a href="{{ route('report.ranpur.pdf', request()->all()) }}" class="btn btn-danger btn-sm ms-2">
                            Export PDF
                        </a>
                    </div>
                </form>
                        ...
                        <div class="modal modal-blur fade" id="modal-edit-{{ $val->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    <form action="{{ route('report.ranpur.update', $val->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="modal-header">
                                            <h5 class="modal-title">Edit Data Ranpur</h5>
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="jenis_kendaraan" class="form-control" value="{{ $val->jenis_kendaraan }}" required>
                                            </div>
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="mb-3">
            ...
            <div class="card-footer d-flex align-items-center">
                ...
                <ul class="pagination m-0 ms-auto">
                    <li class="page-item {{ $report->onFirstPage() ? 'disabled' : '' }}">
                        <a class="page-link" href="{{$controller->prevPagination($report->currentPage(), 'report.ranpur', request()->all())->link}}">
                    ...
                    @foreach($controller->counterPagination($report->lastPage(), $report->currentPage(), 'report.ranpur', request()->all()) as $p)
                    ...
                    <li class="page-item {{ $report->hasMorePages() ? '' : 'disabled' }}">
                        <a class="page-link" href="{{$controller->nextPagination($report->currentPage(), $report->lastPage(), 'report.ranpur', request()->all())->link}}">
                            next
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
